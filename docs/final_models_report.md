...
|:---|:---|:---:|:---:|:---:|:---:|:---:|
| **Small (소형 종양)** | **Attention U-Net (Zoom-Refiner)** | 424 | 0.6980 | **0.6996** | **+0.0016 (+0.16%p)** 🚀 | **9.9209 px** |
| **Medium (중형 종양)** | **UNet++** | 509 | 0.9239 | **0.9240** | **+0.0001 (+0.01%p)** 📈 | **0.8535 px** 🎯 |
| **Large (대형 종양)** | **SegResNet** | 238 | 0.9528 | **0.9529** | **+0.0001 (+0.01%p)** 📈 | **0.3973 px** ⚡ |
...
1. **3-Stage Dynamic Routing 파이프라인의 우수성**:
   - 단일 모델로 모든 종양을 처리할 때 발생하는 소형 종양 왜곡 문제를 해결하기 위해 도입된 크기별 동적 라우팅이 전체 1,171개 슬라이스에서 **평균 DSC 0.8486, HD95 4.0440 px**를 달성했습니다.
2. **크기별 맞춤형 PPO 에이전트 및 Zoom-Refiner의 효과**:
   - **Large 종양(SegResNet)**: **0.9529 DSC (HD95 `0.3973 px`)**의 서브 0.4픽셀 극정밀 오차에 도달했습니다.
   - **Medium 종양(UNet++)**: **0.9240 DSC (HD95 `0.8535 px`)**로 1픽셀 미만의 수직 상향 정밀도를 달성했습니다.
   - **Small 활성 종양(Attention U-Net)**: Zoom-Refiner(4배율 소프트 앙상블) 도입으로 **0.7277 DSC** 고성능에 도달했습니다.
3. **RL-Refiner의 임상적 가치**:
   - Gated Fallback 및 Confidence Guard($\ge 0.90$)를 통해 100% 점수 손실 없이 자율 미세 보정을 완벽하게 완료하였습니다.
