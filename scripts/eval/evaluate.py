...
def evaluate(
    ...
    model_type: str = "unet",   # "unet", "segresnet", "unetplusplus", "attention_unet"
    num_eval: int = 50,
    max_steps: int = 30,
    ...
    in_channels = 2 if modality in ["t1ce+flair", "t1ce+t2"] else 1
    # 세그멘테이션 모델 로드 (있으면)
    unet = None
    if unet_path and os.path.exists(unet_path):
        if model_type == "segresnet":
            unet = build_segresnet(in_channels=in_channels, out_channels=1).to(device)
            log.info(f"SegResNet 로드: {unet_path}")
        elif model_type == "unetplusplus":
            unet = build_unetplusplus(in_channels=in_channels, out_channels=1).to(device)
            log.info(f"UNet++ 로드: {unet_path}")
        elif model_type == "unet3plus":
            unet = build_unet3plus(in_channels=in_channels, out_channels=1, DSV=False).to(device)
            log.info(f"UNet 3+ 로드: {unet_path}")
        elif model_type == "attention_unet":
            unet = build_attention_unet(in_channels=in_channels, out_channels=1).to(device)
            log.info(f"Attention U-Net 로드: {unet_path}")
        else:
            unet = build_unet(in_channels=in_channels, out_channels=1).to(device)
