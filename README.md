...
### 🔬 [신규 전용 폴더] BraTS 서브리전(ET / TC / WT) 기준 3단계 동적 라우팅 파이프라인

독립된 전용 폴더 `subregion_pipeline/`에서 뇌종양 3대 서브리전(**ET**, **TC**, **WT**)의 병리학적 특징에 맞춘 전문가 라우팅 벤치마크 결과입니다. (`subregion_pipeline/run_subregion_pipeline.py` 실행 결과)

#### 📊 서브리전 라우팅 파이프라인 종합 성능

| 항목 | 수치 / 결과 | 비고 |
|:---|:---:|:---|
| **평가 대상 슬라이스** | **1,237 개** | 20명 독립 테스트 환자 전수 평가 |
| **Stage 2 백본 초기 DSC** | **0.8533** | ET/TC/WT 서브리전 라우팅 적용 |
| **Stage 3 PPO Refiner 최종 DSC** | **0.8536** | GT-Free 자율 미세 보정 (최상위 기록 🚀) |
| **최종 경계선 오차 (HD95)** | **2.5356 px** | **서브 2.6픽셀 극소 경계 오차 도달 ⚡** |

#### 🎯 서브리전(Subregion-wise) 세부 성능 비교

| 서브리전 구분 | 매핑된 Expert 백본 | 슬라이스 수 | 초기 DSC | 최종 DSC | **최종 HD95 오차** |
|:---|:---|:---:|:---:|:---:|:---:|
| **ET (Enhancing Tumor)** | **Attention U-Net (Zoom-Refiner)** | 614 | 0.8454 | **0.8454** | **3.1130 px** |
| **TC (Tumor Core)** | **UNet++** | 71 | 0.9460 | **0.9480** 🚀 | **0.3521 px** 🎯 *(서브 0.4px 극정밀)* |
| **WT (Whole Tumor)** | **SegResNet** | 552 | 0.8502 | **0.8505** | **2.1741 px** ⚡ |

#### 🖼️ 서브리전 파이프라인 보정 시각화 샘플
![Subregion Pipeline Sample](results/subregion_pipeline_comparison.png)
