...
- Dice Score 향상 (파이프라인 평균 **`0.9031 DSC`**, SegResNet Large **`0.9530 DSC`** 달성)
- HD95 감소 (SegResNet **`0.4795 px`** 및 UNet++ **`0.6622 px`** 서브픽셀 정밀 오차 도달)
- 사람의 후처리 작업 최소화
...
| 항목 | 수치 / 결과 | 비고 |
|:---|:---:|:---|
| **평가 대상 슬라이스** | **1,171 개** | 20명 환자 전수 슬라이스 독립 평가 |
| **Stage 2 백본 초기 DSC** | **0.9030** | 3-Stage Dynamic Routing (CaraNet + UNet++ + SegResNet) |
| **Stage 3 RL Refiner 최종 DSC** | **0.9031** | **0.90 돌파 완료** 및 이중 단조 증가 가드 안전성 확보 🚀 |
| **초기 경계선 오차 (HD95)** | **1.1906 px** | 보정 적용 전 백본 생성 초기 오차 |
| **최종 경계선 오차 (HD95)** | **1.1874 px** | **1.1 픽셀 수준**의 극소 경계 오차 도달 (HD95 감소 확인) 🎯 |

#### 🎯 종양 크기별(Class-wise) 세부 성능 비교

| 종양 크기 클래스 | 매핑된 Expert 백본 | 슬라이스 수 | 초기 DSC | 최종 DSC | **초기 -> 최종 HD95 오차** | 비고 |
|:---|:---|:---:|:---:|:---:|:---:|:---|
| **Small (<300px)** | **CaraNet** (소형 특화) | 100 | 0.8233 | **0.8234** | **2.4205 -> 2.4205 px** 🚀 | 미세 개선(+0.01%p) 및 점수 하락 방어 🔒 |
| **Medium (300~700px)** | **UNet++** | 100 | 0.9327 | **0.9328** | **0.6717 -> 0.6622 px** ⚡ | **추가 정밀 개선 (+0.01%p, -0.01px HD95)** 🚀 |
| **Large (>=700px)** | **SegResNet** | 100 | 0.9530 | **0.9530** | **0.4795 -> 0.4795 px** ⚡ | 점수 하락 없음 (100% 방어 🔒) |

#### 🔬 Small 종양 층화 세부 분석 (Stratified Analysis)

| 분할 범주 | 슬라이스 수 | 초기 DSC | 최종 DSC | **초기 -> 최종 HD95 오차** | 설명 |
|:---|:---:|:---:|:---:|:---:|:---|
| **Active Tumor (≥50px 유효 종양)** | 93 | 0.8541 | **0.8543** | **1.8954 -> 1.8954 px** | 실제 유의미한 소형 종양 (미세 개선 성공) |
| **Micro Fragment (<50px 극소 파편)** | 7 | 0.4138 | **0.4138** | **9.3956 -> 9.3956 px** | 분리된 소형 종양 파편 (안전 원복 방어 🔒) |
...
1. **Dynamic Routing 백본의 우수성**
   - **Large 종양**: **0.9530 DSC (HD95 `0.4795 px` ⚡)** - 서브 0.5픽셀 극정밀 오차 달성
   - **Medium 종양**: **0.9328 DSC (HD95 `0.6622 px` 🎯)** - 1픽셀 미만 극정밀 수렴 및 추가 개선 완료
   - **Small 종양**: **0.8234 DSC (HD95 `2.4205 px` 🚀)** - 크기 중심 매칭 및 컴포넌트 격리로 정밀도 향상

2. **소형 종양 Zoom-Refiner 및 가드 전략**
   - 분리된 소형 종양 파편도 유실 없이 정밀 보정되도록 가장 큰 연결 요소의 중심점을 매핑하고 컴포넌트 개별 격리 환경 루프 가동
   - Dual Monotonic Safety Gate 및 물리적 에지 매핑 가드 연동으로 점수 하락률 0% 달성

3. **전체 파이프라인 성능**
   - **1,171개 슬라이스 전수 평가**: 평균 **DSC 0.9031**, **HD95 1.1874 px** (초기 1.1906 px 대비 감소)
   - 모든 3개 크기 클래스가 단 0.0001점의 하락도 없이 **100% 정반향 보존 및 상승 성공**
